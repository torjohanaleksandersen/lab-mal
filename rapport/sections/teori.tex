...
\begin{equation}
    ...
    V_{out}(x_{pot}) = V_{in} \cdot  \left(\frac{x_{pot} - R}{x_{pot} + R} \right)
\end{equation}
...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
\caption{Grafen til $V_{out}(x_{pot})$}
\label{fig:wheatstone-graph}
\end{figure}

\vspace{1em}
\noindent
Grafens ekstremalpunkter i intervallet $x_{pot} \in [0, 100\text{k}\Omega]$ vil fortelle oss hva $V_{out}$ sin laveste og høyeste mulig verdi er. Siden grafen oppfører seg som strengt økende, dvs. $V_{out}(s) < V_{out}(s + \Delta s)$ i intervallet $[0, 10^5]$, kan vi garantere at vi finner den laveste verdien i $x_{pot} = 0$ og høyest verdien i $x_{pot} = 10^5$. Dermed løser vi for det som:
\begin{align}
V_{out}(0) &= -12\text{V} \\
